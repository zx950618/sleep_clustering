...
if __name__ == '__main__':
    ...
    # 计算Pearson相关系数
    print("1-st Dimension Pearson correlation:")
    for i in range(len(feature_names)):
        correlation_1, p_1 = pearsonr(sample_days[:, i].flatten(), embedded_data[:, 0].flatten())
        if p_1 <= 0.05:
            print("{}\t{:.2f}\t{}{:.4f}".format(feature_names[i], correlation_1, "" if correlation_1 >= 0 else "-", p_1))
        else:
            print("{}\t \t ".format(feature_names[i]))


    print("2-nd Dimension Pearson correlation:")
    for i in range(len(feature_names)):
        correlation_2, p_2 = pearsonr(sample_days[:, i].flatten(), embedded_data[:, 1].flatten())
        if p_2 <= 0.05:
            print("{}\t{:.2f}\t{}{:.4f}".format(feature_names[i], correlation_2, "" if correlation_2 >= 0 else "-", p_2))
        else:
            print("{}\t \t ".format(feature_names[i]))


    # 计算Spearman相关系数
    print("1-st Dimension Spearman correlation:")
    for i in range(len(feature_names)):
    ...
    print("2-nd Dimension Spearman correlation:")
    for i in range(len(feature_names)):
