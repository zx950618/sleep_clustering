...
class RevisedConvolutionalAutoencoder(object):
    def __init__(self, filter_size, filter_num, encoded_size):
        x = tf.placeholder(tf.float32, shape=[None, 1, 39, 1])

        # encoder part
        conv1 = Convolution2D([filter_size, 39, 1, filter_num], padding='VALID', activation=tf.nn.relu, scope='conv_1')(
            x)
        unfold = Unfold(scope='unfold')(conv1)
        encoded = FullyConnected(encoded_size, activation=tf.nn.relu, scope='encode')(unfold)

        # decoder part
        decoded = FullyConnected((1 - filter_size + 1) * 1 * filter_num, activation=tf.nn.relu, scope='decode')(encoded)
        fold = Fold([-1, (1 - filter_size + 1), 1, filter_num], scope='fold')(decoded)
        reconstruction = DeConvolution2D([filter_size, 39, 1, filter_num],
                                         padding='VALID',
...
raw_data = pd.read_csv("data/data_for_CAE.csv").values
index = raw_data[0:, 0:1]
value = np.concatenate((index, raw_data[0:, 3:42]), axis=1)

X = []
for userid in np.unique(index):
    user_data = value[np.where(value[:, 0] == userid)][:, 1:]
    user_data = user_data.astype(dtype=np.float64)
    X.append(user_data)

new_X = np.array(X).reshape(-1, 39)
...
Sliding_X = Sliding_X.reshape(-1, 1, 39, 1)
...
np.savetxt('results/insomnia_normal_student_{}_latent.csv'.format(conv_autoencoder.name), latent, delimiter=",")
