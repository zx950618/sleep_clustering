...
def mean_value_without_max_and_min(data):
    data = np.asarray(data)
    if data.shape[0] < 5:
        # mean_str = "{:.2f}".format(np.mean(data))
        # std_str = "{}".format(np.std(data))
...
def std_value_without_max_and_min(data):
    data = np.asarray(data)
    if data.shape[0] < 5:
        return np.std(data)
    data = np.sort(data)[1:-1]
...
if __name__ == '__main__':
    ...
    xi_0 = np.squeeze(embedded_data[:, 0])[np.where(sources == 0)[0]]
    xi_1 = np.squeeze(embedded_data[:, 0])[np.where(sources == 1)[0]]
    xi_2 = np.squeeze(embedded_data[:, 0])[np.where(sources == 2)[0]]

    yi_0 = np.squeeze(embedded_data[:, 1])[np.where(sources == 0)[0]]
    yi_1 = np.squeeze(embedded_data[:, 1])[np.where(sources == 1)[0]]
    yi_2 = np.squeeze(embedded_data[:, 1])[np.where(sources == 2)[0]]
    plt.scatter(xi_0, yi_0, marker='o', color=colors[0], label='insomnia')
    plt.scatter(xi_1, yi_1, marker='*', color=colors[1], label='normal')
    plt.scatter(xi_2, yi_2, marker='s', color=colors[2], label='students')
